...
use near_store::{
    DBCol, KeyForStateChanges, ShardTries, Store, StoreUpdate, Temperature, WrappedTrieChanges,
...
impl ChainStore {
    pub fn new(store: Store, temp: Temperature, genesis_height: BlockHeight, save_trie_changes: bool) -> ChainStore {
        ChainStore {
            store,
            temp,
            genesis_height,
    ...
    pub fn iterate_state_sync_infos(&self, temp: Temperature) -> Result<Vec<(CryptoHash, StateSyncInfo)>, Error> {
        self.store
            .iter(temp, DBCol::StateDlInfos)
            .map(|item| match item {
                Ok((k, v)) => Ok((
    ...
    pub fn get_state_changes_for_split_states(
        ...
        option_to_not_found(
            self.store.get_ser(temp, DBCol::StateChangesForSplitStates, key),
...
impl ChainStore {
    ...
    pub fn get_outcomes_by_id(
        ...
        Ok(self
            .store
            .get_ser(temp, DBCol::TransactionResult, id.as_ref())?
            .unwrap_or_else(|| vec![]))
    }
    ...
    pub fn get_outcomes_by_block_hash_and_shard_id(
        ...
        Ok(self
            .store
            .get_ser(temp, DBCol::OutcomeIds, &get_block_shard_id(block_hash, shard_id))?
            .unwrap_or_default())
    }
    ...
    pub fn get_all_block_hashes_by_height(
        ...
        let result = match temp {
            Temperature::Hot => self.read_with_cache(
                DBCol::BlockPerHeight,
                &self.block_hash_per_height,
                &index_to_bytes(height),
            ),
            // Temperature::Cold => read single block from BlockHeight
        };
        option_to_not_found(result, format_args!("BLOCK PER HEIGHT: {}", height))
    }
    ...
    pub fn get_all_chunk_hashes_by_height(
        ...
        Ok(self
            .store
            .get_ser(temp, DBCol::ChunkHashesByHeight, &index_to_bytes(height))?
            .unwrap_or_default())
    }
    ...
    pub fn get_all_header_hashes_by_height(
        ...
        Ok(self
            .store
            .get_ser(temp, DBCol::HeaderHashesByHeight, &index_to_bytes(height))?
            .unwrap_or_default())
    }
    ...
    pub fn get_state_header(
        ...
        match self.store.get_ser(temp, DBCol::StateHeaders, &key) {
    ...
    pub fn get_latest_known(&self, temp: Temperature) -> Result<LatestKnown, Error> {
        self.latest_known
            .get_or_try_init(|| {
                option_to_not_found(
                    self.store.get_ser(temp, DBCol::BlockMisc, LATEST_KNOWN_KEY),
                    "LATEST_KNOWN_KEY",
                )
            })
            .cloned()
    ...
    pub fn get_state_changes_in_block(
        ...
        let mut block_changes = storage_key.find_iter(&self.store, temp);
        ...
        Ok(match state_changes_request {
            StateChangesRequest::AccountChanges { account_ids } => {
                ...
                for account_id in account_ids {
                    ...
                    let changes_per_key = storage_key.find_exact_iter(&self.store, temp);
            ...
            StateChangesRequest::AllAccessKeyChanges { account_ids } => {
                ...
                for account_id in account_ids {
                    ...
                    let changes_per_key_prefix = storage_key.find_iter(&self.store, temp);
    ...
    fn read_with_cache<'a, T: BorshDeserialize + Clone + 'a>(
        ...
        self.read_with_cache_impl(temp, col, cache, key)
    }

    fn read_hot_with_cache<'a, T: BorshDeserialize + Clone + 'a>(
        &self,
        col: DBCol,
        cache: &'a CellLruCache<Vec<u8>, T>,
        key: &[u8],
    ) -> io::Result<Option<T>> {
        self.read_with_cache_impl(Temperature::Hot, col, cache, key)
    }

    fn read_with_cache_impl<'a, T: BorshDeserialize + Clone + 'a>(
        &self,
        temp: Temperature,
        col: DBCol,
        cache: &'a CellLruCache<Vec<u8>, T>,
        ...
        if let Some(value) = cache.get(key) {
            Ok(Some(value))
        } else if let Some(value) = self.store.get_ser(temp, col, key)? {
...
struct ChainStoreReader<'a> {
    pub chain_store: &'a ChainStore,
    pub temp: Temperature,
}

impl<'a> ChainStoreReader<'a> {
    fn new(chain_store: &'a ChainStore, temp: Temperature) -> Self {
        Self { chain_store, temp }
    }
}

impl<'a> ChainStoreAccess for ChainStoreReader<'a> {
    fn store(&self) -> &Store {
    ...
    fn head(&self) -> Result<Tip, Error> {
        if let Some(ref tip) = self.head {
            ...
            option_to_not_found(self.chain_store.store.get_ser(Temperature::Hot, DBCol::BlockMisc, HEAD_KEY), "HEAD")
        }
    }
    ...
    fn tail(&self) -> Result<BlockHeight, Error> {
        if let Some(tail) = self.tail.as_ref() {
            ...
            self.chain_store.store
                .get_ser(Temperature::Hot, DBCol::BlockMisc, TAIL_KEY)
    ...
    fn chunk_tail(&self) -> Result<BlockHeight, Error> {
        self.chain_store.store
            .get_ser(self.temp, DBCol::BlockMisc, CHUNK_TAIL_KEY)
            .map(|option| option.unwrap_or_else(|| self.genesis_height))
            .map_err(|e| e.into())
    }
    ...
    fn fork_tail(&self) -> Result<BlockHeight, Error> {
        self.chain_store.store
            .get_ser(self.temp, DBCol::BlockMisc, FORK_TAIL_KEY)
            .map(|option| option.unwrap_or_else(|| self.genesis_height))
            .map_err(|e| e.into())
    }
    ...
    fn largest_target_height(&self) -> Result<BlockHeight, Error> {
        match self.chain_store.store.get_ser(self.temp, DBCol::BlockMisc, LARGEST_TARGET_HEIGHT_KEY) {
            Ok(Some(o)) => Ok(o),
            Ok(None) => Ok(0),
            Err(e) => Err(e.into()),
        }
    ...
    fn header_head(&self) -> Result<Tip, Error> {
        option_to_not_found(
            self.chain_store.store.get_ser(self.temp, DBCol::BlockMisc, HEADER_HEAD_KEY),
            "HEADER_HEAD",
        )
    }
    ...
    fn final_head(&self) -> Result<Tip, Error> {
        option_to_not_found(
            self.chain_store.store.get_ser(self.temp, DBCol::BlockMisc, FINAL_HEAD_KEY),
            "FINAL HEAD",
        )
    }

    /// Get full block.
    fn get_block(&self, block_hash: &CryptoHash) -> Result<Block, Error> {
        option_to_not_found(
            self.read_with_cache(DBCol::Block, &self.blocks, block_hash.as_ref()),
            format_args!("BLOCK: {}", h),
        )
    }
    ...
    /// Does this full block exist?
    fn block_exists(&self, block_hash: &CryptoHash) -> Result<bool, Error> {
        self.chain_store.store.exists(self.temp, DBCol::Block, block_hash.as_ref()).map_err(|e| e.into())
    }

    fn chunk_exists(&self, chunk_hash: &ChunkHash) -> Result<bool, Error> {
        self.chain_store.store.exists(self.temp, DBCol::Chunks, chunk_hash.as_ref()).map_err(|e| e.into())
    }
    ...
    fn get_block_header(&self, h: &CryptoHash) -> Result<BlockHeader, Error> {
        option_to_not_found(
            self.read_hot_with_cache(DBCol::BlockHeader, &self.headers, h.as_ref()),
    ...
    fn get_block_hash_by_height(&self, height: BlockHeight) -> Result<CryptoHash, Error> {
        option_to_not_found(
            self.chain_store.store.get_ser(Temperature::Hot, DBCol::BlockHeight, &index_to_bytes(height)),
    ...
    fn get_blocks_to_catchup(&self, hash: &CryptoHash) -> Result<Vec<CryptoHash>, Error> {
        match self.temp {
            Temperature::Hot => Ok(self
                .store
                .get_ser(Temperature::Hot, DBCol::BlocksToCatchup, hash.as_ref())?
                .unwrap_or_default()),
            // _ => Ok(Vec::new())
        }
    }
    ...
    fn is_block_challenged(&self, hash: &CryptoHash) -> Result<bool, Error> {
        Ok(self
            .store
            .get_ser(self.temp, DBCol::ChallengedBlocks, hash.as_ref())?
            .unwrap_or_default())
    }
    ...
    fn is_invalid_chunk(
        ...
        match self.temp {
            Temperature::Hot => self
                .read_with_cache(DBCol::InvalidChunks, &self.invalid_chunks, chunk_hash.as_ref())
...
pub struct ChainStoreUpdate<'a> {
    chain_store: ChainStoreReader<'a>,
...
impl<'a> ChainStoreUpdate<'a> {
    pub fn new(chain_store: &'a mut ChainStore, temp: Temperature) -> Self {
        ChainStoreUpdate {
            chain_store: ChainStoreReader::new(chain_store, temp),
            store_updates: vec![],
    ...
    /// Storage temperature; i.e. which storage data is to be read from.
    ///
    /// Since updates are only ever done to hot storage, the assumption here is
    /// that read done through this object are always done against hot storage
    /// as well.
    const TEMP: Temperature = Temperature::Hot;
}
...
impl<'a> ChainStoreUpdate<'a> {
    pub fn get_state_changes_for_split_states(
        ...
        self.chain_store.get_state_changes_for_split_states(Self::TEMP, block_hash, shard_id)
    }
    ...
    pub fn try_save_latest_known(&mut self, height: BlockHeight) -> Result<(), Error> {
        let latest_known = self.chain_store.get_latest_known(Temperature::Hot).ok();
    ...
    pub fn clear_chunk_data_and_headers(
        ...
        for height in chunk_tail..min_chunk_height {
            let chunk_hashes = self.chain_store.get_all_chunk_hashes_by_height(Temperature::Hot, height)?;
            ...
            let header_hashes = self.chain_store.get_all_header_hashes_by_height(Temperature::Hot, height)?;
    ...
    pub fn clear_block_data(
        ...
        for shard_id in 0..block.header().chunk_mask().len() as ShardId {
            ...
            if let Ok(shard_state_header) = self.chain_store.get_state_header(Temperature::Hot, shard_id, block_hash)
            {
                let state_num_parts =
        ...
        let stored_state_changes: Vec<Box<[u8]>> = self
            .chain_store
            .store()
            .iter_prefix(Temperature::Hot, DBCol::StateChanges, storage_key.as_ref())
    ...
    pub fn gc_col_block_per_height(
        ...
        let mut store_update = self.store().store_update();
        let epoch_to_hashes = self.chain_store.get_all_block_hashes_by_height(Temperature::Hot, height)?;
    ...
    pub fn gc_outcomes(&mut self, block: &Block) -> Result<(), Error> {
            ...
            block.chunks().iter().filter(|h| h.height_included() == block.header().height())
        {
            ...
            let outcome_ids =
                self.chain_store.get_outcomes_by_block_hash_and_shard_id(Temperature::Hot, block_hash, shard_id)?;
            for outcome_id in outcome_ids {
                let mut outcomes_with_id = self.chain_store.get_outcomes_by_id(Temperature::Hot, &outcome_id)?;
    ...
    pub fn copy_chain_state_as_of_block(
        ...
        for (shard_id, chunk_header) in block.chunks().iter().enumerate() {
            ...
            let outcome_ids =
                source_store.get_outcomes_by_block_hash_and_shard_id(Self::TEMP, block_hash, shard_id)?;
            for id in outcome_ids.iter() {
                let existing_outcomes = source_store.get_outcomes_by_id(Self::TEMP, id)?;
    ...
    fn finalize(&mut self) -> Result<StoreUpdate, Error> {
        ...
        for (hash, block) in self.chain_store_cache_update.blocks.iter() {
            let mut map =
                match self.chain_store.get_all_block_hashes_by_height(Self::TEMP, block.header().height()) {
        ...
        for (hash, header) in self.chain_store_cache_update.headers.iter() {
            ...
            header_hashes_by_height
                ...
                .or_insert_with(|| {
                    self.chain_store
                        .get_all_header_hashes_by_height(Self::TEMP, header.height())
                        .unwrap_or_default()
                })
                .insert(*hash);
        ...
        for (chunk_hash, chunk) in self.chain_store_cache_update.chunks.iter() {
            ...
            match chunk_hashes_by_height.entry(height_created) {
                ...
                Entry::Vacant(entry) => {
                    let mut hash_set =
                        match self.chain_store.get_all_chunk_hashes_by_height(Self::TEMP, height_created) {
        ...
        for (hash, outcomes) in self.chain_store_cache_update.outcomes.iter() {
            let mut existing_outcomes = self.chain_store.get_outcomes_by_id(Self::TEMP, hash)?;
...
mod tests {
    ...
    use near_store::Temperature::Hot;
    ...
    fn test_clear_old_data() {
        ...
        for i in 0..15 {
            ...
            if i < 8 {
                ...
                assert!(chain
                    .mut_store()
                    .get_all_block_hashes_by_height(Hot, i as BlockHeight)
                    .is_err());
            } else {
                assert!(chain.get_block(blocks[i].hash()).is_ok());
                assert!(chain.mut_store().get_all_block_hashes_by_height(Hot, i as BlockHeight).is_ok());
            }
        }
    ...
    fn test_clear_old_data_fixed_height() {
        ...
        assert_eq!(
            chain
                .mut_store()
                .get_all_block_hashes_by_height(Hot, 5)
                .unwrap()
                .values()
                .flatten()
                .collect::<Vec<_>>(),
        ...
        assert!(chain.mut_store().get_all_block_hashes_by_height(Hot, 4).is_err());
        assert!(chain.mut_store().get_all_block_hashes_by_height(Hot, 5).is_ok());
        assert!(chain.mut_store().get_all_block_hashes_by_height(Hot, 6).is_ok());
